...
also copy the private key ( non-.pub file created with ssh-key-gen ) into `/home/lemmy/.ssh/` directory. You can cat the file contenets and paste then into the server. ( Optional: It is also possible to use instead use scp  `scp {private key} root@{ip-address}:/home/lemmy/.ssh/' replace any variables signified with {} ).
